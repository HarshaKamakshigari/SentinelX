...
Flow (Sequential with conditional skipping):
  Input → Normalizer → Heuristic Risk → Graph Layer → Risk Combiner
    → Orchestrator → Malware → Network → ThreatIntel → VT → Triage → Report → END
...
from ..agents.threatintel_agent import threatintel_node
...
def compile_graph():
    ...
    graph.add_node("threatintel_agent", threatintel_node)
    ...
    graph.add_edge("threatintel_agent", "vt_agent")
